...
osSemaphoreId_t adcPrintSemHandle;
...
int main(void)
{
 ...
 // MX_DFSDM1_Init();
  ...
  /* add semaphores, ... */
  const osSemaphoreAttr_t adcPrintSem_attributes = {
    .name = "adcPrintSem"
  };
  adcPrintSemHandle = osSemaphoreNew(1, 0, &adcPrintSem_attributes);
...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef *hadc) {
    dma_half = 1;
    osSemaphoreRelease(adcPrintSemHandle);
...
void StartDefaultTask(void *argument)
{
  ...
  /* Infinite loop */
    for(;;)
    {
        if (osSemaphoreAcquire(adcPrintSemHandle, osWaitForever) == osOK)
        {
            if (dma_half == 1)
            {
